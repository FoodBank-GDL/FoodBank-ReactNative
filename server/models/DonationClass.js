...
class DonationClass {
  static async createDonation(body) {
    try {
      ...
      //Aqui hago cosas
      const campaignDocumentRef = doc(db, "campaigns", body.campaignId);
      const campaignDocument = await getDoc(campaignDocumentRef);
      const campaignData = campaignDocument.data();
      await Firestore.updateDoc(campaignDocument.ref, {
        donativosTotales: campaignData.donativosTotales + 1,
      });
  ...
  static async getCampaignDonations(campaignId) {
    try {
      ...
      const campaignDonationsQuerySnap = await Firestore.getDocs(
        campaignDonationsQuery
      );
      const campaignDonationsObject = campaignDonationsQuerySnap.docs.map(
        (doc) => doc.data()
      );
  ...
  static async changeUserDonationStatus(body) {
    try {
      ...
      const userDonationsQuerySnap = await Firestore.getDocs(
        userDonationsQuery
      );
      ...
      const userDonationsObject = userDonationsQuerySnap.docs.map((doc) =>
        doc.data()
      );
